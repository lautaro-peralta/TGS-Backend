...
interface ClienteDB extends RowDataPacket {
  id: string;
  nombre: string;
  email: string | null;
  direccion: string | null;
  telefono: string | null;
  regCompras: string | null;
}

export class ClienteRepository implements Repository<Cliente> {
  
  private mapDbToEntity(row: ClienteDB): Cliente {
    return Cliente.fromDbRow(row);
  }

  public async findAll(): Promise<Cliente[]> {
    try {
      const [rows] = await pool.query<ClienteDB[]>('SELECT * FROM clientes');
  ...
  public async findOne(id: string): Promise<Cliente> {
    try {
      const [rows] = await pool.query<ClienteDB[]>(
  ...
  public async add(clienteInput: Omit<Cliente, 'id'>): Promise<Cliente> {
    try {
        const id = uuidv4();  // Genero el UUID en Node.js
        console.log('clienteInput recibido en add:', clienteInput);
        await pool.query<ResultSetHeader>(
            `INSERT INTO clientes (id, nombre, email, direccion, telefono, regCompras)
             VALUES (?, ?, ?, ?, ?, ?)`,
            [
                id,
                clienteInput.nombre,
  ...
  public async update(id: string, clienteData: Partial<Cliente>): Promise<Cliente> {
    try {
  ...
  public async delete(id: string): Promise<Cliente> {
    try {
