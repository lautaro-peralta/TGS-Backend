...
export class AdminController {
  ...
  async createAdmin(req: Request, res: Response) {
    ...
    try {
      ...
      await em.transactional(async (txEm) => {
        ...
        const adminExists = await txEm.findOne(Admin, { dni: validatedData.dni });
        if (adminExists) {
          throw new Error('ADMIN_ALREADY_EXISTS');
        }

        // 2️⃣ Find the user by DNI (via person relationship)
        const user = await txEm.findOne(User, { person: { dni: validatedData.dni } }, {
          populate: ['person']
        });
        ...
        let basePerson = await txEm.findOne(BasePersonEntity, { dni: validatedData.dni });
        ...
        await txEm.flush();
        responseData = admin.toDTO();
      });

      return ResponseUtil.created(
        res, 
        `Admin created successfully and ADMIN role assigned to user ${userName}`,
        responseData
      );
    } catch (err: any) {
      ...
      if (err.message === 'USER_NOT_FOUND') {
        return ResponseUtil.notFound(res, 'User', validatedData.dni);
      }
  ...
  async deleteAdmin(req: Request, res: Response) {
    ...
    try {
      ...
      await em.transactional(async (txEm) => {
        const admin = await txEm.findOne(Admin, { dni });
        if (!admin) {
      ...
      return ResponseUtil.deleted(
        res, 
        userName 
          ? `Admin deleted successfully and ADMIN role removed from user ${userName}`
